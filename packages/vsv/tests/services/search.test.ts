...
import { buildSearchIndex, filterSearch } from '../../src/services/search';
...
const getEnvComment = (envId: string) => envId === 'prod' ? 'Production' : '';
...
describe('buildSearchIndex', () => {
  ...
  it('handles empty vault data', () => {
    const emptyData: VaultData = { version: 1, settings: { autolockMinutes: 5 }, services: {}, environments: {}, secrets: {}, templates: { main: {} } };
    const idx = buildSearchIndex(emptyData);
    expect(idx).toEqual([]);
  });
});
