// generate.ts — generate .env section

import * as vault from '../vault';
import { generateEnv } from 'vsv/services/env-generator';
import { esc, selectedEnv, setSelectedEnv } from './helpers';
import { renderButton } from './components/button';
import { renderEnvPills, bindEnvPills } from './components/env-pills';
import type { EnvEntry } from 'vsv/types/vault';
...
function renderTable(entries: EnvEntry[]): string {
  ...
  return `
    <table class="w-full text-sm border-collapse table-fixed">
      ...
      <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
        ${entries.map(({ key, value, source, secret }) => `
        <tr>
...
function renderRaw(output: string): string {
...
export function renderGenerate(render: () => void): string {
  ...
  return `
    <div class="max-w-3xl">
      ...
      ${selectedEnv ? `<div class="flex gap-3 mb-4">
...
export function bindGenerate(render: () => void): void {
  bindEnvPills((envId) => {
