        ...
        <div class="flex flex-wrap gap-2 items-end">
          ...
          <div class="flex-1 min-w-[200px]">
            ...
            <div class="flex gap-1">
              <input id="new-secret-value" type="password" placeholder="Valeur" class="flex-1 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none" />
              ${renderButton(icons.refresh(), { id: 'btn-gen-secret', variant: 'secondary', cls: '!px-2', title: 'Generer' })}
            </div>
          </div>
          <div class="flex items-end gap-2">
            ...
            ${renderButton('Ajouter', { id: 'btn-add-secret', variant: 'primary' })}
          </div>
        </div>
      </div>
...
export function bindSecrets(render) {
  ...
  // Toggle input type based on secret checkbox
  document.getElementById('new-secret-is-secret').onchange = (e) => {
    document.getElementById('new-secret-value').type = e.target.checked ? 'password' : 'text';
  };
  ...
  bindEditableRows('[data-edit-secret]', (row) => {
    ...
    startInlineEdit(row, {
      ...
      onSave: async (newValue) => {
        await vault.setSecret(level, serviceId, field, newValue, isSecret);
        render();
      },
      onCancel: render,
    });
